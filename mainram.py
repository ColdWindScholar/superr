...
def ramdisk(j, base, func, romname, filetype, action, extra):
    ...
    def byname1(keep=None):
        if ffstab == 1:
            return
        bytest = {'/KERNEL': 'kerbyname', '/system': 'regbyname',
                  '/SYSTEM': 'capbyname', '/APP': 'appbyname', '/userdata': 'datbyname'}
    ...
    def deopatch():
        with j.cd(rd):
            sepol = j.greps(
                j.fl('', '.*test|.*txt|.*mapping|.*00_project_files'), j.findr('**/*sepolicy*'))
            if sepol:
                for i in sepol:
                    omd5 = j.md5chk(i)
                    if i.endswith('.cil'):
                        line = j.greps(
                            '\(allow zygote.* dalvikcache_data_file.* \(file \(', j.readfl(i))
                        if line:
                            for e in line:
                                if not 'execute' in e:
                                    nline = e.replace(')))', ' execute)))')
                                    j.sedf(
                                        e.replace('(', '\(').replace(')', '\)'), nline, i)
                        else:
                            zname = j.grepf('\(allow zygote.*', i)
                            dname = j.grepf(
                                '\(allow .*dalvikcache_data_file.*', i)
                            if zname and dname:
                                znames = []
                                for z in zname:
                                    line = z.split()[1]
                                ...
                                for z in znames:
                                    for d in dnames:
                                        j.appendf(
                                            '(allow '+z+' '+d+' (file (execute)))', i)
                    else:
                        dumpf = j.cmd(seinject+' -dt -P '+i).splitlines()
                        dname = j.greps(
                            'ALLOW.*dalvikcache_data_file.*', dumpf)
                        ...
                        if zname and dname:
                            ...
                            for z in znames:
                                for d in dnames:
                                    j.cmd(seinject+' -s '+z+' -t '+d
                                          + ' -c file -p execute -P '+i+' -o '+i)
                        else:
                            j.cmd(
    ...
    def forcee():
        ...
        for ff in ffstab:
            ...
            fetest = j.greps(
                '.*forceencrypt|.*forcefdeorfbe|.*fileencryption', fetmp)
    ...
    def insecure(filekind):
        ...
        if insecstatus == 'No':
            ...
            if j.grepf('ro.adb.secure=1', dfprop):
                ...
                j.awkadd('ro.secure=0', 'ro.adb.secure=0',
                         'after', 'last', dfprop)
    ...
    def isdmverity():
        ...
        if isver or vdtb == 'yes':
            j.appendf('Yes='+color['r']+lang['enabled']
                      + color['n'], prfiles+'/statusfile')
        else:
            j.appendf('No='+color['g']+lang['disabled']
                      + color['n'], prfiles+'/statusfile')
    ...
    def isinsecure():
        if j.existf(dfprop):
            if not j.grepf('ro.secure=0', dfprop):
                j.appendf('No='+color['r']+lang['secure']
                          + color['n'], prfiles+'/statusfile')
            else:
                j.appendf('Yes='+color['g']+lang['insecure']
                          + color['n'], prfiles+'/statusfile')
        else:
            j.appendf('No='+color['r']+'N/A'+color['n'], prfiles+'/statusfile')
    ...
    def mmc():
        ...
        for i in ['/system', '/boot', '/data', '/cache', '/recovery', '/modem', '/vendor']:
            ...
            if j.greps('/dev/', cline):
                j.appendf(j.greps('/dev/', cline)[0]
                          + ' '+i, deviceloc+'/superr_mmc')
            elif j.greps(i[1:], cline):
                j.appendf(j.greps(i[1:], cline)[0]
                          + ' '+i, deviceloc+'/superr_mmc')
