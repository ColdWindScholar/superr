import struct, hashlib, bz2, sys, lzma, update_metadata_pb2 as um
...
def main(filename, *parts):
    ...
    for part in dam.partitions:
        if parts and part.partition_name not in parts:
            pass
        else:
            for op in part.operations:
                if not op.type in (op.REPLACE, op.REPLACE_BZ, op.REPLACE_XZ):
