...
def cmd(command):
    if platf == 'win':
        ...
        test1 = Popen(command, close_fds=True, shell=True, stdin=PIPE, stdout=PIPE, stderr=STDOUT,
...
def deodex_start(quiet=None):
                ...
                with cd('squashfs-root'):
                    ...
                    with cd(odexarch):
                        for i in findf('*'):
                            ...
                            os.replace(sqshdir + '/squashfs-root/' + odexapp + '/' + odexapp + '.apk',
                                       sqshdir + '/applications/' + odexapp + '/' + odexapp + '.apk')
    ...
    def deodex(dtype):
        ...
        def dodeodex(deoarch, deoarch2, deoappdir):
            for app in applist:
                ...
                with cd(framedir + '/' + deoarch2):
                    if dtype in ['m2', 'n2']:
                        classes = cmd(
                            'java -Xmx' + heapsize + 'm -jar ' + baksmali + ' list dex ' + app2 + '.odex').splitlines()
                        ...
                        for line in classes:
                            ...
                            appendf(
                                cmd('java -Xmx' + heapsize + 'm -jar ' + baksmali + ' deodex -b boot.oat ' + app2 + '.odex/' + apkdex + ' -o smali'),
                                logs + '/deodex.log')
                            if findf('smali/*'):
                                appendf(
                                    cmd('java -Xmx' + heapsize + 'm -jar ' + smali + ' assemble -a ' + api + ' smali -o ' + dexclass),
                                    logs + '/deodex.log')
                            delpath('smali')
                            if not existf(dexclass):
                                appendf(deoappdir + '/' + app + '/' + deoarch + '/' + dexclass,
                        ...
                        appendf(cmd('java -Xmx' + heapsize + 'm -jar ' + oat2dex + ' ' + app2 + '.odex odex'),
                                logs + '/deodex.log')
                        ...
                        for i in findf('*_classes*.cdex'):
                            if platf == 'win':
                                ...
                                appendf('\n'.join(grepv('^==', cmd(vdexcon + ' ' + i).splitlines())),
                                        logs + '/deodex.log')
                                if existf(i + '.new'):
                                    os.replace(i + '.new', i[:-5] + '.dex')
                                    delpath(i)
        ...
        with cd(rd):
            odextmp = findr('system/**/*.odex') + findr('vendor/**/*.odex') + findr('oem/**/*.odex') + findr(
                'product/**/*.odex') + findr('system_ext/**/*.odex')
        if dtype != 'l':
            if sar():
                odextmp = greps(fl('',
                                   'system/system/framework/oat/|system/system/framework/' + arch + '/|system/system/framework/' + arch2 + '/|system/system/app|system/system/priv-app|system/system/vendor/framework'),
                                odextmp)
            else:
                odextmp = greps(fl('',
                                   'system/framework/oat/|system/framework/' + arch + '/|system/framework/' + arch2 + '/|system/app|system/priv-app|system/vendor/framework'),
                                odextmp)
        else:
            odextmp = greps(
                fl('', 'system/framework/' + arch + '/|system/framework/' + arch2 + '/|system/app/|system/priv-app/'),
                odextmp)
        ...
        if chimerao:
            ...
            for i in chimerao:
                ...
                if existf('/'.join([dname, 'oat', darch, aname + '.vdex'])):
                    os.replace(i.replace('.odex', '.vdex'),
                               '/'.join([sysdir, 'app', aname, 'oat', darch, aname + '.vdex']))
            ...
            if existd(framedir + '/' + deoarch):
                with cd(framedir + '/' + deoarch):
                    ...
                    for frame in frametmp:
                        ...
                        with cd(framedir + '/' + deoarch2):
                            appendf(cmd('java -Xmx' + heapsize + 'm -jar ' + oat2dex + ' ' + frame + ' odex'),
                                    logs + '/deodex.log')
                            delpath(frame)
                            for i in findf('*.dex'):
        ...
        with cd(rd):
            stillodexed = '\n'.join(
                findr('system/**/*.odex') + findr('vendor/**/*.odex') + findr('oem/**/*.odex') + findr(
                    'product/**/*.odex') + findr('system_ext/**/*.odex'))
        if not stillodexed:
            if dtype == 'n2':
                delete_meta_inf()
                ...
                with cd(dirname(mainfile)):
                    ...
                    for cdex in findf('*_classes*.cdex'):
                        if platf == 'win':
                            ...
                            appendf('\n'.join(grepv('^==', cmd(vdexcon + ' ' + cdex).splitlines())),
                                    logs + '/deodex.log')
    ...
    def deodex_old():
        ...
        def dodeodexold():
            for app in applist:
                ...
                appendf(
                    cmd('java -Xmx' + heapsize + 'm -jar ' + baksmali + ' -a ' + api + ' -d ' + framedir + ' -x ' + deoappdir + os.sep + app + '.odex -o ' + deoappdir + os.sep + 'smali'),
                    logs + '/deodex.log')
                if findf(deoappdir + os.sep + 'smali/*'):
                    appendf(
                        cmd('java -Xmx' + heapsize + 'm -jar ' + smali + ' -a ' + api + ' ' + deoappdir + os.sep + 'smali -o ' + deoappdir + os.sep + 'classes.dex'),
                        logs + '/deodex.log')
                    appendf(
                        cmd(aapt + ' add -fk ' + deoappdir + os.sep + app + '.' + deoext + ' ' + deoappdir + os.sep + 'classes.dex'),
                        logs + '/deodex.log')
...
def dllang():
    ...
    with cd(tools + '/language'):
        ...
        try:
            ...
            langdl = chlist(color['gb'] + 'Choose language to download:' + color['n'], sorted(list(langlist)),
                            len(list(langlist)))
...
def ext4Xtract(whatimg, *vargs):
    def ext4Xtract_o():
                    ...
                    if not vargs or 'e' in vargs:
                        if platf == 'win':
                            if entry_inode_path.endswith('.'):
                                appendf(
                                    'Directory renamed without trailing . due to NTFS restrictions:\n' + entry_inode_path,
                            ...
                            try:
                                ...
                                try:
                                    ...
                                    if link_target and all(c in string.printable for c in link_target):
                                        ...
                                        appendf('Failed symlink: ' + whatimg + entry_inode_path,
                                                logs + '/ext4_extract.log')

                                        continue
                                except:
                                    appendf('Failed symlink 2: ' + whatimg + entry_inode_path,
                                            logs + '/ext4_extract.log')
                    ...
                    if dirr:
                        ...
                        if not vargs or 'f' in vargs:
                            if platf == 'win':
                                if entry_type == 7:
                                    fsconfig.append(
                                        whatimg + entry_inode_path + '.sym.srk ' + uid + ' ' + gid + ' ' + mode + cap)
                                elif wdone:
                                    fsconfig.append(whatimg + entry_inode_path + '.ex' + str(
                                        cnt) + '.srk ' + uid + ' ' + gid + ' ' + mode + cap)
                                else:
                                    fsconfig.append(
                        ...
                        if not vargs or 'f' in vargs:
                            if platf == 'win':
                                if entry_type == 7:
                                    fsconfig.append(
                                        entry_inode_path[1:] + '.sym.srk ' + uid + ' ' + gid + ' ' + mode + cap)
                                elif wdone:
                                    fsconfig.append(entry_inode_path[1:] + '.ex' + str(
                                        cnt) + '.srk ' + uid + ' ' + gid + ' ' + mode + cap)
                                else:
        ...
        with open(whatimg + '.img', 'rb') as f:
            ...
            if not vargs or ('f' in vargs or 'c' in vargs or 's' in vargs):
                ...
                if dirr:
                    try:
                        fsconfig = [whatimg + ' ' + str(root.inode.i_uid) + ' ' + str(root.inode.i_gid) + ' ' + getperm(
                            root.mode_str)]
                    except:
        ...
        delpath(prfiles + '/symlinks-' + whatimg, prfiles + '/fs_config-' + whatimg,
                prfiles + '/file_contexts3-' + whatimg)

        if not vargs or 's' in vargs:
            for i in sorted(symlinks):
                try:
                    appendf('symlink("' + i.split()[0] + '", "/' + i.split()[1] + '");',
                            prfiles + '/symlinks-' + whatimg)
                except:
                    pass
        ...
        if not vargs or 'c' in vargs:
            if not greps('/' + whatimg + ' ', contexts):
                ...
                try:
                    rootcon = '/' + whatimg + ' ' + \
                              greps('/' + whatimg + '/bin |/' + whatimg + '/lib ', contexts)[0].split()[1]
                except:
                    pass
    ...
    def ext4Xtract_m():
        ...
        if existf(tools + '/source/ramdiskl.py'):
            print(
                cmd('sudo ' + tools + '/source/ramdiskl.py ' + tools + '/source/getmeta output ' + whatimg + ' ' + prfiles))
        else:
            appendf(
                cmd('sudo ' + tools + '/source/superr --otherfile ' + tools + '/source/getmeta output ' + whatimg + ' ' + prfiles),
                logs + '/ext4_extract.log')
    ...
    def ext4Xtract_7():
        ...
        delpath(whatimg, prfiles + '/symlinks-' + whatimg, prfiles + '/fs_config-' + whatimg,
                prfiles + '/file_contexts3-' + whatimg)
...
def get_contexts():
    if not existf(prfiles + '/file_contexts2'):
        ...
        if existf(prfiles + c):
            ...
            with cd(rd):
                flist = findr('system/**') + findr('vendor/**') + findr('oem/**') + findr('product/**') + findr(
                    'system_ext/**')
...
def get_devicename():
    ...
    if getconf('devicename', uconf):
        ...
        devtmp = ['ro.product.device', 'ro.build.product', 'ro.product.name', 'ro.product.system.device',
...
def line_end(infile, outfile):
    with open(infile, 'rb') as f, open(outfile, 'wb') as o:
        for i in f:
            o.write(i.replace(b'\x0D\x0A', b'\x0A'))
...
def partimg(whatimg, sparseimg='', fromplug=None, frommenu=None, quiet=None):
    ...
    if not imgsize:
        ...
        size_mess = whatimg + '.img ' + lang['img_fail_fit'] + '\nimg size: ' + str(
            round(int(imgsize) / 1024 / 1024)) + 'MB, dir size: ' + str(round(dirsize / 1024 / 1024)) + 'MB'
    ...
    with cd(rd):
        ...
        kprint(whatimg + ': ' + imgsize + ' bytes (' + str(rd(int(imgsize) / 1024 / 1024)) + ' MB) ...', 'y')
        ...
        if 'ext4_allocate_best_fit_partial:' in tmpbuild.split():
            appendf('[INFO] Out of space in ext4 ' + whatimg + '_new.img file. Trying again as ext2.',
                    logs + '/img_build.log')
...
def partimg2(whatimg, sparseimg=None, fromplug=None, frommenu=None, quiet=None):
    ...
    with cd(rd):
        ...
        complete_features = ['dir_prealloc', 'has_journal', 'imagic_inodes', 'ext_attr', 'dir_index', 'resize_inode',
                             ...
                             'quota', 'bigalloc', 'metadata_csum', 'replica', 'read-only', 'project', 'verity',
                             'compression', 'filetype', 'needs_recovery', 'journal_dev', 'extent', 'extents', 'meta_bg',
                             '64bit', 'mmp', 'flex_bg', 'ea_inode', 'dirdata', 'metadata_csum_seed', 'large_dir',
                             'inline_data', 'encrypt', 'casefold', 'fname_encoding']
        ...
        if existf(whatimg + '.img'):
            if platf in ['wsl', 'wsl2', 'lin']:
                ...
                try:
                    ...
                    tune_dict = {}
                    appendf('[INFO] tune2fs did not find features in your existing system.img. Using defaults.',
                            logs + '/img_build.log')
        ...
        if whatimg == 'system' and sar():
            build_args += ['-K', 'yPmy6kiDfsWLtMoM', '-P', prfiles, '-X', whatimg, '-L', whatimg, '-C',
                           prfiles + '/fs_config-' + whatimg, whatimg, whatimg + '_new.img', 'ext4', '/', imgsize,
                           prfiles + '/file_contexts_min']
        else:
            build_args += ['-K', 'yPmy6kiDfsWLtMoM', '-P', prfiles, '-X', whatimg, '-L', whatimg, '-C',
                           prfiles + '/fs_config-' + whatimg, whatimg, whatimg + '_new.img', 'ext4', '/' + whatimg,
                           imgsize, prfiles + '/file_contexts_min']
...
def plug_update(plugins, getlist=None, quiet=None):
    ...
    if upplugs:
        ...
        with cd(tools + '/plugins'):
            for i in upplugs:
                delpath(i)
                dlfile('https://github.com/ColdWindScholar/superr_files/raw/plugins/' + i + '.zip', i + '.zip')
